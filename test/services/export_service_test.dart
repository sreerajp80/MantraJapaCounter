...
void main() {
  ...
  group('ExportService Tests', () {
    ...
    test('importFromJson detects encrypted content and throws EncryptedExportException', () async {
      ...
      final encrypted = await encryptionService.encrypt(plaintext, 'secretKey123');
    ...
    test('importEncryptedFromJson decrypts and imports data successfully', () async {
      const exportData = ExportData(
        exportDate: 12345,
        counters: [counter1],
        sessions: [],
    ...
    test('importSelectedFromJson only imports chosen counters', () async {
      ...
      final exportData = ExportData(
        exportDate: 12345,
        counters: [counter1.copyWith(name: 'Modified C1'), counter2],
        sessions: [],
      );

      await exportService.importSelectedFromJson(
      ...
      // c1 untouched
      expect(counters.firstWhere((c) => c.id == 'c1').name, equals('Om Namah Shivaya'));
      // c2 added
      expect(counters.firstWhere((c) => c.id == 'c2').name, equals('Hare Krishna'));
    });

    test('importSelectedEncryptedFromJson decrypts and selectively imports', () async {
      await repo.insertCounter(counter1);
